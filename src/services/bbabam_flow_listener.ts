...
abstract class BBabamFlowServiceListener {
  ...
  constructor() {
    this.socket = io("http://http://43.201.32.219:4827");
    ...
    this.socket.on('poi_generation', (data) => {
      this.onPoiGeneration(
        data.place_keywords,
        data.place_crawled_data
      );
    });
    this.socket.on('path_generation', (data) => {
      this.onPathGeneration(
        data.path_keywords,
        data.path_crawled_data
      );
    });
  }
  ...
  abstract onPoiGeneration(placeKeywords:string[], placeCrawledData:POIData[]):void;
  abstract onPathGeneration(pathKeywords:string[], pathCrawledData:POIData[]):void;
}
